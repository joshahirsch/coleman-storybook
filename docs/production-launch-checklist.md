...
- Real, owner-approved Camp Coleman brand tokens (navy/teal/blue, Montserrat/Open Sans, real logo) — no longer placeholder. `npm run brand:update` applies them non-destructively. See DL-013.
...
- [ ] `npm run brand:update` run once to apply the real Camp Coleman brand tokens — see DL-013. Without this, pages render with `globals.css`'s fallback defaults rather than the DB-driven values (they currently happen to match, but production shouldn't rely on that).
...
The application itself is built, tested, and reviewed — CI now runs the full check suite automatically on every push, database migrations are versioned and committed, a production build compiles cleanly, and the app now carries real, owner-approved Camp Coleman branding rather than placeholders. With transcription/AI explicitly deferred, the infra stack decided (Vercel + Supabase, no dedicated domain yet), and legal review consciously deferred for the MVP pilot (DL-010), the remaining path to a real POC launch is now just: provision Vercel + Supabase, run `npm run db:migrate`, `npm run content:bootstrap`, and `npm run brand:update` against the real database, smoke-test the (already-written) storage adapter against the real bucket, and run `npm run admin:create` for Josh Hirsch's real account. `docs/phase-14-provisioning-runbook.md` walks through all of this in order. No further product or process decisions are blocking — this is now purely execution, pending your go-ahead to actually provision production infrastructure (Phase 14).
