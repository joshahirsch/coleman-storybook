...
| Contributor entering a fake/mistyped/someone-else's email at submission time | A 6-digit one-time code is emailed to the address and must be entered back before recording can start; the code is HMAC-hashed at rest (never stored plaintext), single-use, expires after 10 minutes, and is locked out after 5 wrong attempts | `src/lib/auth/otp.ts`, `src/lib/actions/public-actions.ts` (`sendVerificationCodeAction`/`verifyEmailCodeAction`) |
| Client claiming "the email code was correct" without it actually being checked server-side | `startSubmissionAction` independently re-validates a signed, short-lived (15 min), email-bound HMAC token — the same "never trust the client's claim alone" principle as upload confirmation above — rather than trusting that the browser only calls it after a real verification | `src/lib/auth/verification-token.ts` |
| Using the OTP-send endpoint as a spam relay against a third party's inbox, or hammering "resend" | Rate-limited both per-IP and per-email (separately) via the same in-memory limiter other public endpoints use | `src/lib/actions/public-actions.ts`, `src/lib/rate-limit.ts` |

## Authentication and authorization

- **Admin surface only.** There is no contributor-facing login/account system — contributors are identified by name/relationship at submission time, not authenticated accounts, consistent with the spec's "no contributor account system" scope. **As of 2026-08-12, the email a contributor provides IS verified** (a one-time code sent to it must be entered back before recording starts, see the threat table above) — this proves email ownership for that one submission, but is a single-use verification step, not a persistent login/session/account. Anyone with a campaign link can still start a submission; nothing about starting one exposes any other contributor's data.
...
- **`EMAIL_PROVIDER` defaults to `"console"`** (logs the OTP code instead of emailing it) if unset — a safe default for dev, but a real footgun if forgotten in a production deploy: contributors would see "code sent" with no way to ever receive it. Double-check `EMAIL_PROVIDER=resend` (plus `RESEND_API_KEY`/`RESEND_FROM_EMAIL`) is actually set in Vercel production before relying on this in the live pilot — see `docs/resend-setup.md`.
- **Email OTP verification proves the contributor received that one code at that one address**, not that the name attached to it is real — someone could still enter a disposable/temporary email they briefly control and receive the code there. This raises the bar against typos and casual impersonation (the stated goal, per the contact-log-CSV/CRM use case) but is not identity verification in a stronger sense, and was never intended to be.
- **Resend integration is not yet verified against the live API** — this session's checks covered typecheck/lint/tests/build against a local clone only; the sandbox that built it has no network path to Resend's API (same restriction as Supabase/Google Drive elsewhere in this doc). A real end-to-end send needs to be confirmed once deployed — see `docs/resend-setup.md` step 5.
